    ...
    <main className="min-h-screen bg-transparent pt-12 md:pt-16 pb-12 md:pb-16 px-4 md:px-8 overflow-x-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4">
              {summaryCards.map((card, idx) => (
                <div key={idx} className="group p-5 rounded-2xl
                  ...
                  <h3 className="text-sm font-bold mb-1 text-slate-900 dark:text-white">{card.title}</h3>
      ...
      <section className="container mx-auto max-w-4xl mb-24 md:mb-36 px-2 md:px-0">
        <div className="text-center mb-12 md:mb-16">
          <span className="text-blue-600 dark:text-blue-400 font-mono tracking-[0.3em] uppercase text-[10px] font-bold block mb-3">Milestones</span>
          <h2 className="text-3xl md:text-5xl font-bold tracking-tight text-[#111827] dark:text-white">Career Evolution</h2>
          <div className="mt-4 w-12 h-1 bg-blue-600 dark:bg-blue-400 mx-auto rounded-full"></div>
        </div>

        <div className="space-y-8 md:space-y-10 relative">
          <div className="absolute left-[-20px] top-4 bottom-4 w-[1px] hidden md:block
            bg-gradient-to-b from-blue-500/40 via-purple-400/30 to-transparent">
          </div>
          ...
          {workDetails.map((work, index) => (
            <div
              ...
              className="relative p-7 md:p-10 rounded-[2rem] md:rounded-[2.5rem]
                border border-slate-200/90 dark:border-white/10
                bg-white dark:bg-[#18181B]
                hover:shadow-xl hover:shadow-slate-200/60 dark:hover:shadow-none
                transition-all duration-300 group overflow-hidden"
            >
              <div className="absolute left-[-24px] top-12 w-2 h-2 rounded-full bg-blue-600 hidden md:block shadow-[0_0_12px_rgba(37,99,235,0.8)]"></div>
                ...
                <div className="space-y-2">
                  <span className={`inline-flex items-center gap-1.5 font-mono text-[9px] md:text-[10px] uppercase tracking-widest px-3 py-1 rounded-full font-semibold border ${
                    work.Type === "Achievement"
                      ...
                      : "bg-slate-100 dark:bg-blue-500/15 text-slate-800 dark:text-blue-300 border-slate-200 dark:border-blue-500/30"
                  }`}>
                  ...
                  <p className="text-slate-600 dark:text-stone-400 text-base md:text-lg font-medium">{work.Company}</p>
                </div>
                <div className="md:text-right space-y-1.5">
                  <div className="flex items-center text-slate-600 dark:text-stone-400 text-xs font-mono md:justify-end">
                    <Calendar size={12} className="mr-2 text-blue-600" />
                    {work.Duration}
                  </div>
                  <div className="flex items-center text-slate-500 dark:text-stone-500 text-xs md:justify-end">
              ...
              <div className="space-y-3 mb-8">
                {(work.Desc || []).map((bullet, bIndex) => (
                  <div
                    key={bIndex}
                    className="flex items-start text-slate-600 dark:text-stone-300 text-sm md:text-base leading-relaxed"
                  >
                    <ArrowRight
                      size={14}
                      className="text-blue-600 dark:text-blue-400 mt-1 mr-3 shrink-0"
                    />
                    <span>{bullet}</span>
                  </div>
                ))}
      ...
      <section className="container mx-auto max-w-5xl mb-12">
        <div className="flex flex-col md:flex-row items-center md:items-end justify-between mb-12
          border-b border-slate-200 dark:border-white/10 pb-8 gap-6 text-center md:text-left">
          <div className="w-full">
            <span className="text-blue-600 dark:text-blue-400 font-mono tracking-[0.3em] uppercase text-[10px] font-bold block mb-2">Knowledge Base</span>
            <h2 className="text-3xl md:text-5xl font-bold tracking-tight text-[#111827] dark:text-white">Academic Path</h2>
          </div>
          <BookOpen className="text-slate-300 dark:text-white/10 hidden lg:block shrink-0" size={60} strokeWidth={1.5} />
        </div>
            ...
            >
              <div className="flex items-center justify-between mb-8">
                ...
                <div className="text-right">
                  ...
                  <span className="text-[10px] text-slate-400 dark:text-stone-500 uppercase tracking-widest">{edu.Location}</span>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <p className="text-slate-600 dark:text-stone-400 text-base font-medium">{edu.Company}</p>

                <div className="pt-6 flex items-center justify-between
                  border-t border-slate-200 dark:border-white/5 mt-6">
                  <div className="flex flex-col">
                    <span className="text-[9px] uppercase text-slate-400 dark:text-stone-500 tracking-[0.2em] font-mono font-bold mb-1">Achievement</span>
                  ...
                  <div className="w-9 h-9 rounded-full
                    ...
                    flex items-center justify-center
                    text-slate-600 dark:text-stone-400
                    group-hover:bg-[#111827] dark:group-hover:bg-white
