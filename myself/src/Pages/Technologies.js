  ...
  return (
    <main className="min-h-screen bg-transparent pt-24 pb-16 px-6">
      ...
      <div className="container mx-auto max-w-6xl text-center mb-24">
        <span className="text-blue-600 dark:text-blue-500 font-black tracking-[0.4em] uppercase text-[10px] block mb-4">
          Technical Foundation
        </span>
        <h1 className="text-5xl md:text-7xl leading-[1.15] pb-2 font-bold bg-clip-text text-transparent bg-gradient-to-b from-gray-900 to-gray-500 dark:from-white dark:to-white/40 mb-6">
          Technology Stack
        </h1>
        <p className="text-gray-500 dark:text-gray-500 max-w-2xl mx-auto text-sm leading-relaxed mb-10">
...
<div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-6 md:gap-8 lg:gap-18">
  {expertiseDomains.map((domain, idx) => (
    <div
      key={idx}
      className="group p-6 rounded-[2rem] bg-white dark:bg-white/[0.02] border border-gray-200 dark:border-white/5 hover:border-gray-300 dark:hover:border-white/10 hover:bg-gray-50 dark:hover:bg-white/[0.04] transition-all duration-500 relative overflow-hidden shadow-sm dark:shadow-none flex flex-col h-full"
    >
      ...
      <section className="container mx-auto max-w-7xl">
        <div className="flex items-center gap-4 mb-16 px-4">
          ...
          <h2 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Technical Skillset</h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {categories.map((cat, idx) => (
            <div
              key={idx}
              className="p-8 rounded-[2rem] bg-white dark:bg-white/[0.01] border border-gray-200 dark:border-white/5 hover:border-gray-300 dark:hover:border-white/10 transition-all group relative overflow-hidden shadow-sm dark:shadow-none"
            >
              <div className={`absolute inset-0 bg-gradient-to-br from-${cat.color}-500/[0.05] dark:from-${cat.color}-500/[0.02] to-transparent opacity-0 group-hover:opacity-100 transition-opacity`}></div>
              ...
              <div className="relative z-10">
                ...
                <div className="flex flex-wrap gap-2">
                  {cat.items.map((item, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-2.5 px-3 py-2 bg-gray-100 dark:bg-white/[0.03] border border-gray-200 dark:border-white/5 rounded-xl text-xs font-medium text-gray-600 dark:text-gray-500 group-hover:text-gray-900 dark:group-hover:text-gray-300 group-hover:bg-white dark:group-hover:bg-white/[0.06] group-hover:border-gray-300 dark:group-hover:border-white/10 transition-all"
                    >
                      {item.src ? (
